...
<script>
	import { range, rangedLerp } from './js/utility.js';
...
<main>
	<div id="clock-container">
		...
		<div id="analog-clock" style="height:25%;">
			...
			<!-- FIXME: Fix the strip alarm marker showing, the markers doesn't align with what they represent -->
			...
			<div id="strip-clock">
				<svg viewBox='-{(stripClockWidth+4)/2} 0 {stripClockWidth+4} 54' width='100%' height='100%'>
					...
					{#each range(0, 60, 1) as marker}
					{#if ((marker + clock.second_precise) % 60) < 15 || ((marker + clock.second_precise) % 60) >= 45}
					<line class='marker-{marker % 5 == 0 ? "large" : "small"} {clock.alarmIsActive && (marker == clock.alarmSecond ? "alarm-marker" : "")}'
							y1='{marker % 5 == 0 ? 6 : 7}'
							y2='10'
							x1='{rangedLerp((marker + clock.second_precise + 30) % 60, 60, 0, -stripClockWidth, stripClockWidth)}'
							x2='{rangedLerp((marker + clock.second_precise + 30) % 60, 60, 0, -stripClockWidth, stripClockWidth)}' />
						{#if marker % 5 == 0}
							<text 
								class="clock-strip-number"
								text-anchor='middle' 
								y='5'
								x='{rangedLerp((marker + clock.second_precise + 30) % 60, 60, 0, -stripClockWidth, stripClockWidth)}'>
								<!-- For some reason the markers are inversed decrease to the right instead of increasing -->
								{rangedLerp(marker, 0, 60, 60, 0) % 60}
						</text>
						{/if}
					{/if}
				{/each}
				...
				{#each range(0, 60, 1) as marker}
					{#if ((marker + clock.minute_precise) % 60) < 15 || ((marker + clock.minute_precise) % 60) >= 45}
						<line class='marker-{marker % 5 == 0 ? "large" : "small"} {clock.alarmIsActive && (marker == clock.alarmMinute ? "alarm-marker" : "")}'
							y1='{marker % 5 == 0 ? 16 : 17}'
							y2='20'
							x1='{rangedLerp((marker + clock.minute_precise + 30) % 60, 60, 0, -stripClockWidth, stripClockWidth)}'
							x2='{rangedLerp((marker + clock.minute_precise + 30) % 60, 60, 0, -stripClockWidth, stripClockWidth)}' />
						{#if marker % 5 == 0}
							<text 
								class="clock-strip-number"
								text-anchor='middle' 
								y='15'
								x='{rangedLerp((marker + clock.minute_precise + 30) % 60, 60, 0, -stripClockWidth, stripClockWidth)}'>
								<!-- For some reason the markers are inversed decrease to the right instead of increasing -->
								{rangedLerp(marker, 0, 60, 60, 0) % 60}
						</text>
						{/if}
				...
				{#each range(0, 24*2, 1) as marker}
					{#if ((marker + clock.hour_precise) % (24*2)) < (6*2) || ((marker + clock.hour_precise) % (24*2)) >= (9*2)}
						<line class='marker-{marker % 2 == 0 ? "large" : "small"} {clock.alarmIsActive && (marker == clock.alarmHour ? "alarm-marker" : "")}'
							y1='{marker % 2 == 0 ? 26 : 27}'
							y2='30'
							x1='{rangedLerp((marker + clock.hour_precise + 12*2) % (24*2), 24*2, 0, -stripClockWidth, stripClockWidth)}'
							x2='{rangedLerp((marker + clock.hour_precise + 12*2) % (24*2), 24*2, 0, -stripClockWidth, stripClockWidth)}' />
						{#if marker % 2 == 0}
							<text 
								class="clock-strip-number"
								text-anchor='middle' 
								y='25'
								x='{rangedLerp((marker + clock.hour_precise + 12*2) % (24*2), (24*2), 0, -stripClockWidth, stripClockWidth)}'>
								<!-- For some reason the markers are inversed decrease to the right instead of increasing -->
								{rangedLerp(marker, 0, 24*2, 24, 0)}
						</text>
						{/if}
					{/if}
				{/each}
